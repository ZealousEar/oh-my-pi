...
The threshold pass also carries the per-turn prompt-cache guard (`cacheWarmSuffixTokens`, `8_000` tokens; `0` for prefix-bound thinking models): a result with more than that many newer message tokens sits in the warm, already-sent prefix and is skipped. Because an age-based victim needs at least `40_000` newer tool-output tokens, no age-based victim can pass that guard, so in practice the per-turn threshold pass never writes `[Output truncated - N tokens]`; old outputs are reclaimed recoverably by the shake method (artifact-linked placeholders) or by compaction, both of which rebuild the cache anyway. The age-based policy above remains the behaviour of `pruneToolOutputs` when it is called without the guard.
...
2. Serialize with `serializeConversation()`. Each tool result is capped at 2,000 characters for the summarizer — the first 1,200 and the last 800, joined by `[... N characters truncated from the middle ...]` — so the verdict a tool prints last (failing assertion, `N pass, M fail`, exit status) reaches the summary alongside the head.
