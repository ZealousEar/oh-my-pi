...
- takes precedence over existing CLI overlays (`--config`) and runtime overrides,
- preserves the existing `retry.modelFallback` policy, including an explicit `false`, and
- is never written to global or project config.

Applying is an exact replacement, not a merge: omitted roles remain absent across settings reloads, project changes, and cwd-scoped clones; `retry.fallbackChains` and `cycleOrder` take exactly the preset's values. Later explicit routing edits retain normal persistence and immediately update the active plan without resurrecting unrelated lower-layer roles or chains. An explicit runtime `override` or `clearOverride` releases that path's preset linkage. The hub freezes input, including Escape, while a preset is applying. A rejected or failed live switch restores the runtime routing snapshot; restoring the previous live model and thinking level is best-effort and cannot generally undo provider or session side effects. See [Model presets](./models.md#model-presets) for validation, matching, and concurrent-save semantics.
...
| `computer.task.maxActions` | number | `20` | Actions one `computer.task(...)` run may dispatch before it stops as `exhausted`. |
| `computer.task.maxCalls` | number | `50` | Judgment calls one `computer.task(...)` run may spend (observation decisions, value inference, verification). |
| `computer.task.deadlineSec` | number | `120` | Wall-clock bound for one `computer.task(...)` run. |
| `computer.task.allowConsequential` | boolean | `false` | Let `computer.task` press controls whose label reads as consequential (submit, pay, delete, send, …), commits a sheet/dialog/alert, or cannot be read at face value (mixed-script or invisible characters); otherwise such steps are withheld and the run returns `blocked`. User-level only: project layers are ignored with a warning. |
| `computer.driverBin` | string | unset | Absolute path to the Cua Driver binary (relative paths are ignored); by default detection looks for `$CUA_DRIVER_BIN`, `/Applications/CuaDriver.app/Contents/MacOS/cua-driver`, then `cua-driver` on `PATH`. omp never starts the daemon. User-level only: project layers are ignored with a warning. |
| `computer.task.backend` | `auto` \| `native` \| `cua` | `auto` | Backend for `computer.task`: `auto` uses the Cua Driver only when installed (>= 0.28.2), its daemon is running, and both macOS grants are present, else native accessibility; `native` never probes the driver; `cua` fails closed listing the missing prerequisites. User-level only. |
| `computer.cua.telemetry` | `off` \| `driver` | `off` | `off` sets `CUA_DRIVER_RS_TELEMETRY_ENABLED=false` on every cua-driver process omp spawns (the driver's documented per-process override; nothing global is changed); `driver` leaves the driver's own persisted preference in force. User-level only. |
...
| `providers.judgmentFallback`        | enum    | `llm`     | What a failed TypeSafe judgment (network error, 5xx after retries, rejected key) does next. `llm` re-asks the online chat chain (`tiny`, `smol`, `default`, then the active session model) and marks the answer's provenance with `fallback`. `none` fails closed: the TypeSafe error propagates to the feature, the failed attempt stays on the loop meter with its `error`, and no chat model is called (no chat spend). Only consulted when TypeSafe is the judgment backend. |
| `providers.typesafeModel`           | string  | unset     | Pin the TypeSafe model used for typed judgments (`recommend_skills`, `semantic_find`, `tab.task`, `computer.task`, staging, auto-thinking) so decisions are reproducible; the answering backend and model are reported on every judgment regardless.                                                                                                                                                                                        |
...
- Goal-directed page and window loops: `browser.task.maxActions` (25), `browser.task.maxCalls` (60), `browser.task.deadlineSec` (120), `browser.task.allowConsequential` (false) bound `tab.task(...)` exactly as the `computer.task.*` keys above bound `computer.task(...)`.
- Skill recommendation (`recommend_skills`): `skills.recommend.enabled` (true), `skills.recommend.maxCandidatesPerRequest` (200, clamped to `3..254`: windows are balanced so none holds a single skill, and one slot of the 255-option judgment cap is reserved for the `none` choice), `skills.recommend.minRelevance` (0.1), `skills.recommend.cacheEntries` (64).
- Semantic passage retrieval (`semantic_find`): `semanticFind.maxFiles` (24), `semanticFind.maxBytesPerFile` (262144), `semanticFind.maxPassages` (2000), `semanticFind.passagesPerRequest` (200), `semanticFind.contextLines` (2). Caps fail closed with a message naming what was not read.
- Pane subagents: `task.paneBackend` (`native` default; `auto` uses a HerdR pane only when `omp` itself runs inside one; `herdr` requires `task.herdr.session`, never `default`), `task.herdr.session` (unset), `task.herdr.readyTimeoutMs` (30000), `task.herdr.promptTimeoutMs` (600000), `task.herdr.keepPane` (false). `OMP_TASK_HERDR=0` disables the backend for a run. See [task](tools/task.md#pane-backend-herdr).
