...
- Later explicit role, fallback-chain, cycle, and thinking edits retain their normal persistence behavior and update the active routing plan immediately. Loading itself does not persist routing. An explicit runtime `override` or `clearOverride` releases preset linkage for that path.
