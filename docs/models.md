...
It does **not** capture enabled models, provider order or provider config, or credentials. Those remain whatever your persisted layers resolve to; a preset only re-points roles and their routing.
...
- Roles omitted from the preset resolve to **auto-selection**. Application is an exact replacement of the role layer, not a merge — omitted roles are tombstoned to auto rather than leaking a value from a lower layer, and those tombstones are durable: later single-role edits never silently resurrect a lower-layer value.
- Fallback chains and the cycle order are also **replaced exactly**: a preset with no chains clears every chain for the session instead of merging with chains from lower layers.
- Application is **transactional and validated**: a malformed or unsupported-version preset is rejected outright, and before anything changes every captured role must resolve to an available, authenticated model (role aliases like `@slow` resolve against the preset's own role map). If validation fails, **nothing changes**; if the live model switch afterwards is rejected or throws, the whole application — settings and live session — is **rolled back**.
...
Preset **definitions** live in the `modelPresets` setting: the `/models` view saves and deletes them per key in the **global** layer (other presets are never rewritten), while definitions from project or overlay layers are also listed and remain visible after a global delete. See [Settings](./settings.md#models) for the persisted shape versus runtime-override application.
