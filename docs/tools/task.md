...
`task.paneBackend` selects where a structured subagent executes after policy, artifact, and id resolution (`runStructuredSubagent`); native in-process execution is the default and remains the fallback whenever a pane is not requested.

- `native` (default): the in-process `AgentSession` path described above.
- `auto`: uses a HerdR pane only when `omp` itself runs inside one (real `HERDR_ENV=1` / `HERDR_PANE_ID`, never the client-only socket variables); otherwise native.
- `herdr`, or a per-item `visible: true`: a strict request. Preflight verifies the `herdr` binary, a running server for `task.herdr.session` (the literal `default` session is refused), and the omp integration: `herdr integration status` must report the extension at `<agentDir>/extensions/herdr-omp-agent-state.ts` for the agent dir the child will run with (this process's agent dir, passed as `PI_CODING_AGENT_DIR`); an install for any other directory fails preflight naming both paths and the exact `PI_CODING_AGENT_DIR=<dir> herdr integration install omp` command. Any failure is an explicit error naming the prerequisite. Preflight also rejects, before any HerdR call, a structured `outputSchema`, an isolated worktree, eval-defined custom tools, a nested-spawn allowlist, proxied parent MCP tools, an attached advisor, a prewalk hand-off, and a restricted parent session (plan mode or an explicit host tool allowlist — a fresh pane omp has no restricted mode, so it would discover extensions, MCP, and LSP the parent never granted).

HerdR identifies the pane's process by name to confirm readiness, so when `omp` is a wrapper script that execs `bun`, set `HERDR_AGENT=omp` in the wrapper's environment; otherwise `agent start --kind omp` times out even though the CLI is up.

Lifecycle: inside a pane the backend runs `pane split --current` (direction from `pane layout`, `--no-focus`); with an explicit session it creates a workspace labelled `omp-task-<id>`. It then runs `agent start <name> --kind omp --pane <id>`, one `agent prompt --wait` carrying the static brief from `prompts/task/pane-subagent.md`, collects the result file the child was told to write at `<artifactsDir>/<id>.md` (so `agent://<id>` resolves exactly as for native runs), and closes only the pane or workspace it created. Progress is reported as `creating-pane → starting-agent → working → idle → ready-for-review`, with `blocked-needs-user` when HerdR reports the agent blocked and `needs-attention` when it settled without a complete result; every pane result states that an idle pane is not proof that verification passed. Each transition is published through the ordinary task progress channel, so a background (async / eval `agent()`) pane run is watched by its enclosing managed job — the backend arms no timer of its own. Worker identity is scoped by HerdR session and socket path, so pane ids from different sessions never collide. Pane children cannot be revived or messaged through `agent://<id>` writes, and their usage stays in the pane session — both are disclosed on the result. `OMP_TASK_HERDR=0` disables the backend for a run.

Rules: a pane child rediscovers unscoped rules from disk itself, but as `main`, so the parent's rules scoped to this agent (`agents:` frontmatter) would never reach it. The parent buckets its own discovery result exactly as an in-process child would (same disable/builtin/TTSR precedence) and ships only the agent-scoped always-apply bodies in the prompt's `# Rules` section; the section is omitted when none apply.

Child contract: the child is the same `omp` CLI, launched with the same contract the native executor computes — the full model pattern list (`--model` for one selector, `--models a,b` for a fallback chain), the per-spawn `effort` mapped onto the canonical thinking range (`--thinking`; the child clamps it to its model) or else the agent's static level, and the normalised tool allowlist (`task` added/removed by depth, `exec` expanded to `eval`/`bash`; an explicit empty allowlist launches the child with `--no-tools`). Because the child starts at depth 0 in its own process, it receives a `PI_CONFIG_FILES` overlay (`<artifactsDir>/<id>.pane-config.yml`) lowering `task.maxRecursionDepth` to the budget the native child would have had; an agent without a spawn policy gets `0`, so it cannot spawn at all.

Result identity: before launch any file at `<artifactsDir>/<id>.md` is removed (a file that cannot be removed fails the run before any pane is created). The delivery protocol requires the child to write `resolved-model: <provider>/<model>` as the first line and `<!-- omp-pane-result:end -->` as the last; both are stripped from `output`. A file is accepted only when it was written after launch, starts with the header, and ends with the sentinel. Every pane result carries `resultSource`: `artifact-file` (the delivered file; the only successful outcome), `pane-read` (the child went idle without a file — `exitCode: 1`, `error: "child went idle without delivering a result file"`, the transcript attached to `stderr` as evidence, nothing written at the artifact path), or `none` (nothing usable: a file without the sentinel is reported as partial, a sealed file without the header as unattributed — both fail with `exitCode: 1`, empty `output`, and the file text in `stderr` as evidence). `resolvedModel` is set only from the header, with `resolvedModelVerified: true`; failures carry `resolvedModelVerified: false` and the requested selector is never assumed. Cancellation is honoured at every stage (pane creation, `agent start`, the prompt wait, and the result settle window, which re-checks the signal every 250 ms): the run reports `aborted: true`, sends `ctrl+c`, waits a bounded `agent wait --until idle`, and closes only the owned pane or workspace (unless `task.herdr.keepPane`); an abort never turns into a success.
