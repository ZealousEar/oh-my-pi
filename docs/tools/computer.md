...
### Structured observation and `computer.task`

- `win.observe({ maxNodes? }) -> { window, nodes[], nodeCount, truncated }` is the structured, read-only counterpart of `win.ax()`: each node carries `ref`, `role`, `title`, `value`, `enabled`, `focused`, global `x/y/width/height`, and its native `actions`. `window` is `null` once the window is gone.
- `await computer.task({ goal, window | app, values?, expect?, maxActions?, maxCalls?, timeout?, allowConsequential? })` pursues one goal in one explicitly targeted window: it observes the accessibility tree, derives the allowed actions locally (set value, perform action, press), has the judgment model pick one per step, re-locates the target by role and label immediately before dispatch (refs are snapshot-scoped on both backends, so a stale ref is re-resolved rather than reused), dispatches, and re-observes. The target must be explicit — an exact window id, a unique `{ app, title }` filter, or `window: "focused"`; an ambiguous filter throws listing the candidates. `values` are the only values the loop may type; a field with no supplied value is derived by the small-model route (metered against `maxCalls`, recorded in `attempts`) or the step is rejected. `status: "done"` is reported only when a caller `expect` (`titleIncludes`/`find`) holds on an independent re-observation; a completion the judge alone corroborates is `unverified` (`verification.verified: "unknown"`). Other statuses are `blocked`, `abstained`, `unsupported`, `exhausted`, and `failed` (a judge/backend error after the loop started; `attempts` are still returned). Node values shown to any model are capped at 200 characters and secure/password fields never expose a value. Bounds: `computer.task.*` settings.
- Question design: each step is one judgment request with a single `select` Choice whose options are the locally derived candidates (`a1…aN`, rubric = the action label) plus the reserved `reobserve`, `wait`, `done`, `blocked`, `abstain` (the no-match/abstain outcomes). The state carries only what the decision needs — `goal`, `window`, `observation { nodes, nodeCount, truncated }` (no revision or digest), `candidateWindow`, `withheld`, `alreadySatisfied`, `authorizedValues`, `nodes` (labels and capped values), `candidates`, `history` — and the instructions name those paths. A `done` claim re-observes; without a caller `expect` the loop asks `goal_satisfied` (a yes/no Noul over `window` and `nodes`, with `true`/`false` criteria) in its own request, and `noul ≥ 0.5` yields `unverified`, never `done`. Choice `confidence` is never a gate.
- Consequential gate: a step is withheld unless the call passes `allowConsequential: true` when its own label names a send/delete/pay-class effect, when the label mixes letter scripts or carries invisible characters (NFKC-normalised first), or when it commits a sheet/dialog/alert — generic confirm labels (`OK`, `Don't Save`, `Replace`, `Delete`, `Close Tab`, …) inside such a container, or any non-dismiss button of a dialog whose text reads as destructive. Dialog text is read from the native backend's static-text nodes; the Cua backend's structured elements omit static text, so there only the container ancestry of actionable rows is available.
- Backends (`computer.task.backend`): `native` drives the session's own accessibility worker and never probes the driver; `cua` requires the [Cua Driver](https://github.com/trycua/cua) and fails closed listing every missing prerequisite; `auto` (default) uses the driver only when it is installed, at least version 0.28.2, its daemon is already running, and both macOS grants are present, otherwise native. The result's `backend` carries `kind` (`native` | `cua`), `mode`, `reason`, the detection, and, when the driver is installed, `version` and the `permissions` snapshot the selection used.
- Cua backend contract (documented against Cua Driver 0.28.2; unverified on a real driver — none is installed on the development host): every call is a one-shot `cua-driver call <tool> '<json>'` against the user's daemon (`list_windows`; `get_window_state { pid, window_id, include_screenshot: false, max_elements }`; `click { target: { kind: "window", pid, window_id }, element_token, delivery_mode: "background", action? }`; `set_value { pid, window_id, element_token, value }`). Requests are validated against those schemas before anything is spawned; `element_token`s come from the immediately preceding snapshot; the driver's closed action result maps `confirmed`/`suspected_noop` → `applied`, `unverifiable`/`partial` → `unknown` (reconciled by re-observation, never retried), refusals (`stale_element_token`, `window_id_not_found`, …) → `stale`, other refusals and daemon/gate errors → `rejected`. `focus` and `scroll-to-visible` have no driver counterpart and are rejected, never approximated. No screenshot is ever requested.
- Permission modes (driver semantics): observing the tree needs Accessibility; every input tool needs Accessibility and Screen Recording because the standard daemon refuses all tool calls (`permissions_pending`) until both grants exist; screenshots need Screen Recording. omp never triggers a TCC prompt: detection is three read-only probes (`--version`, `status`, `permissions status --json`, text form also parsed), and remediation text points at the driver's own `open -n -g -a CuaDriver --args serve` + `cua-driver permissions grant` flow.
- Telemetry (`computer.cua.telemetry`, default `off`): omp sets `CUA_DRIVER_RS_TELEMETRY_ENABLED=false` on every driver process it spawns (probes and calls) — the driver's documented per-process override; `driver` leaves the driver's persisted preference in force. No global preference or rc file is changed.
- `computer.capabilities()` additionally reports `driver`: `{ installed: false, searched[] }` or `{ installed: true, path, version?, daemonRunning, permissions: { accessibility, screenRecording }, modes: { observe, act, screenshot }, usable, blockers[], remediation? }` for the Cua Driver, probed read-only at an absolute `computer.driverBin`, an absolute `CUA_DRIVER_BIN`, `/Applications/CuaDriver.app/Contents/MacOS/cua-driver`, then `cua-driver` on `PATH` (relative configured paths are ignored with a warning). The agent never starts, installs, or upgrades the daemon.
- `computer.enabled`, `computer.driverBin`, `computer.task.allowConsequential`, `computer.task.backend`, and `computer.cua.telemetry` are user-level-only: values in a project's `.omp/config.yml`, `.omp/settings.json`, or `.claude/settings.json` are ignored with a logged warning.
