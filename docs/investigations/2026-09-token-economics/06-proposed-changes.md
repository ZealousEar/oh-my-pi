...
| # | change | effect | evidence | status |
|---|---|---|---|---|
| B10 | **Anthropic 1 h cache TTL on OAuth** — resolved empirically (`08-b10-cache-ttl.md`): the running build (v18.1.10) defaulted OAuth retention to `short`, so breakpoints went out without `ttl` and the server billed `ephemeral_5m`; upstream `6019d73540` (first in **v18.1.18**) defaults OAuth to `long`, and a wire capture from the 18.2.4 source shows `ttl: "1h"` on every breakpoint with the server reporting `ephemeral_1h_input_tokens`. Fix = rebuild from ≥ v18.1.18 (fork `main` is now 18.2.4). Also landed in the same range: tool-array breakpoint on OAuth (`40fbd0e028`), historical decimation breakpoints (`05cf924144`), cache anchors preserved across context hooks (`77f0ff3575`) | 35–43 M write tokens / 14 d (≈ $1k nominal, 19 % of spend is cache-write); 66 % of 15–60 min resumes rebuilt the prefix | measured (wire + server usage) | **resolved by upgrade**; verify with `scripts/analyze.py` cache-write table after a week on ≥ 18.1.18 |
