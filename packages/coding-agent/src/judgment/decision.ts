 ...
 * whether its probabilities are a native distribution (System One) or a
 * synthetic one-hot from a chat keyword (see `TextJudge`), whether the role
 * chain fell through a failed candidate, and the monetary cost when known —
 * `"unknown"` otherwise.
 */
import {
	type ChoiceAnswer,
...
import { type ChainJudgeOptions, type JudgeKind, type JudgmentAttempt, kindOfApi } from "./index";

/**
 * A judge a decision loop can meter: reports each transport attempt through
 * {@link ChainJudgeOptions.onAttempt}. `ChainJudge` is one; a single resolved
 * backend may fix `kind` so failed calls still name the backend that was asked.
 */
export interface DecisionJudge extends Judge {
	/** Fixed backend kind; a role chain leaves it unset and reports the answering candidate's kind per result. */
	readonly kind?: JudgeKind;
	/** Exact model the configuration pins the judge to, when one is configured. */
	readonly pinnedModel?: string;
	judge<Q extends Questions>(request: JudgmentRequest<Q>, options?: ChainJudgeOptions): Promise<JudgmentResult<Q>>;
}

/** Documented option cap of a System One `Choice` question. */
...
export interface DecisionProvenance {
	/** Backend that answered (`native`, `local`, `online`); `unknown` when the call failed before any backend was identified. */
	backend: JudgeKind | "unknown";
	label: string;
	/** Transport/provider/model that actually answered. */
	api: string;
	provider: string;
	model: string;
	/** `native` probabilities from System One; `synthetic` one-hot from a chat keyword. */
	distribution: "native" | "synthetic";
	/** Set when an earlier candidate failed and the chain fell through to the one that answered. */
	fallback?: { from: string; reason?: string };
	/** Model the caller pinned for reproducibility, when one was configured. */
	pinnedModel?: string;
	usage: Usage;
	/** Monetary cost in USD when the transport prices tokens; `unknown` for unpriced native usage. */
	 ...
	 * logical call (a failed candidate before the chain moved on, a chat
	 * completion whose answer was rejected). The answering attempt is the row
	 * without it.
	 */
	nested?: true;
...
export function provenanceOf(
	...
	pinnedModel?: string,
	fallback?: DecisionProvenance["fallback"],
): DecisionProvenance {
	const backend = kindOfApi(result.api);
	...
	return {
		...
		pinnedModel,
		usage: result.usage,
		// Native routes report token counts; a priced route (catalog cost,
		// OpenRouter billing) carries its total, an unpriced one is unknown.
		costUsd: native && result.usage.cost.total === 0 ? "unknown" : result.usage.cost.total,
...
function nestedProvenance(
	...
	const backend = kindOfApi(event.api);
	const native = backend === "native";
	return {
		backend,
		label: judge.label,
		...
		pinnedModel,
		usage: event.usage,
		costUsd: native && event.usage.cost.total === 0 ? "unknown" : event.usage.cost.total,
 ...
 * per failed underlying transport attempt the judge reports (a candidate the
 * chain moved past, a chat completion whose answer was rejected). Features
...
export async function judgeWithMeter<Q extends Questions>(
	...
	// The chain fell through when a failed attempt on another model preceded the answer.
	const fellFrom = superseded.find(
		event => event.error !== undefined && `${event.provider}/${event.model}` !== `${result.provider}/${result.model}`,
	);
	const provenance = provenanceOf(
		...
		{ startedAt, attempt },
		pinnedModel,
		fellFrom ? { from: `${fellFrom.provider}/${fellFrom.model}`, reason: fellFrom.error } : undefined,
	);
