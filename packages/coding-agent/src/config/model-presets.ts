...
import { type ModelRoleLookup, resolveModelRoleValue } from "./model-resolver";
...
export function validateModelPreset(
	...
	for (const [role, value] of Object.entries(preset.roles)) {
		...
		const resolved = resolveModelRoleValue(value, availableModels, { roleLookup });
		if (!resolved.model) {
 ...
 * throw at/after the commit rolls the runtime-override layer and the live
 * session back to the snapshot. All writes target the runtime-override layer
 * only — nothing is persisted to project or global settings.
 */
