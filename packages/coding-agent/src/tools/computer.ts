...
export interface ComputerTaskFactories {
	createJudge?: (session: ToolSession) => DecisionJudge;
...
async function focusedWindowForAuthorization(
	...
	const timeoutSeconds = clampTimeout("computer", 10, cfgToolsMaxTimeout.get(session.settings));
...
async function resolveDriverDetection(
	...
	return await detectCuaDriver({
		driverBin: cfgComputerDriverBin.get(session.settings),
		telemetry: cfgComputerCuaTelemetry.get(session.settings),
...
async function resolveDesktopBackend(
	...
	const mode = cfgComputerTaskBackend.get(settings);
	...
	if (backend.kind === "cua" && driver.installed) {
		return {
			...
			surface: new CuaDesktopSurface({
				...
				env: cuaSpawnEnv(cfgComputerCuaTelemetry.get(settings)),
				exec: factories.driverExec?.(session),
				timeoutMs: remaining,
			}),
		};
...
async function runDesktopTask(
	...
	const onUsage = journalJudgmentUsage(session.sessionManager, "computer-task");
	...
	const timeoutSeconds = clampTimeout(
		"computer",
		params.timeout ?? cfgComputerTaskDeadlineSec.get(settings),
		cfgToolsMaxTimeout.get(settings),
	);
	const budget: LoopBudget = {
		maxCalls: Math.max(1, Math.trunc(params.maxCalls ?? cfgComputerTaskMaxCalls.get(settings))),
		maxActions: Math.max(1, Math.trunc(params.maxActions ?? cfgComputerTaskMaxActions.get(settings))),
	...
	const result = await runComputerTask(
		{
			...
			allowConsequential: params.allowConsequential ?? cfgComputerTaskAllowConsequential.get(settings),
		},
		{
			surface,
			judge,
