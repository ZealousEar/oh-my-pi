 ...
 *
 * Ownership (persistent-agent-browser wave, 2026-09-19): this module is owned
 * by the permission worker. The lifecycle worker only imports
 * {@link decideAutomationAction} / {@link AutomationAction} and calls it before
 * every dispatch. Exported names and signatures below are the frozen contract.
 */
import { rmSync } from "node:fs";
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { acquireFileLock, getGlobalDaemonRuntimeDir } from "@oh-my-pi/pi-utils";
