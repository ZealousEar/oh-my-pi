...
/**
 * Judge identity, taken from the attempt that actually answered when there is
 * one. A role chain fixes no backend kind up front, so the kind is the
 * answering attempt's; `unknown` when no attempt ever named a backend.
 */
function judgeInfo(judge: DecisionJudge, attempts: readonly DecisionProvenance[]): ComputerTaskJudgeInfo {
	const last = attempts.at(-1);
	const kind = last && last.backend !== "unknown" ? last.backend : (judge.kind ?? "unknown");
	return {
		kind,
		label: judge.label,
		model: last?.model ?? judge.pinnedModel ?? "unresolved",
		distribution: last?.distribution ?? (kind === "native" ? "native" : "synthetic"),
