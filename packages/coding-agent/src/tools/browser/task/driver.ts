 ...
 * unfreeze, cancellation, per-call deadline, and per-dispatch permission gate
 * instead of opening a second path to the page: every request carries the
 * policy descriptor the supervisor decides before dispatch.
 */
import { ToolError } from "@oh-my-pi/pi-tui/tools/tool-errors";
import { isRecord } from "@oh-my-pi/pi-utils";
import type { ToolSession } from "../../index";
import { type AutomationAction, fingerprintAutomationValue } from "../../automation-policy";
import { renderFunctionRun } from "../../run-code";
import { originOf } from "../tab-protocol";
...
function readAutomation(name: string, action: string, summary: string, invocationId?: string): AutomationAction {
	return {
		...
		tier: "read",
		action,
		target: originOf(getTab(name)?.info.url),
		consequential: false,
		raw: false,
		summary,
