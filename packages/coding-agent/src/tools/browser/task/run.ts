...
import { type JudgmentUsage, type ResolvedJudge, resolveJudge } from "../../../judgment/index";
...
/** Record every judgment/helper call on the session ledger when one is reachable. */
function usageRecorder(session: ToolSession): ((usage: JudgmentUsage) => void) | undefined {
	const manager = session.sessionManager;
	const append = manager?.appendModelUsage;
	const leafId = manager?.getLeafId;
	const sessionId = manager?.getSessionId;
	if (!manager || !append || !leafId || !sessionId) return undefined;
	const owner = { sessionId: sessionId.call(manager), parentId: leafId.call(manager) };
	return usage => {
		const entryId = append.call(manager, { purpose: "browser-task", ...usage }, owner);
...
export async function executeBrowserTask(session: ToolSession, params: ExecuteTaskParams): Promise<BrowserTaskResult> {
	...
	const onUsage = usageRecorder(session);
