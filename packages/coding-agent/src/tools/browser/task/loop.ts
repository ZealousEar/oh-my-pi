...
export async function runBrowserTask(deps: BrowserTaskDeps): Promise<BrowserTaskResult> {
	...
	return {
		...
		// A role chain has no fixed kind: the backend that answered the last
		// judgment names it; a judge with a fixed kind names itself even when
		// every call failed before a backend was identified.
		backend: {
			kind:
				lastProvenance && lastProvenance.backend !== "unknown" ? lastProvenance.backend : (judge.kind ?? "unknown"),
			label: judge.label,
			...(lastProvenance
				? {
