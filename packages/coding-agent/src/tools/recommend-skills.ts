...
import { cfgSkillsEnabled, cfgSkillsRecommendEnabled } from "../extensibility/settings";
import {
	JUDGE_DESCRIPTION_CHARS,
	judgmentConfigDigest,
	NO_MATCH_CAVEAT,
...
export class RecommendSkillsTool implements AgentTool<typeof recommendSkillsSchema, RecommendSkillsDetails> {
	...
	constructor(session: ToolSession, recommender?: SkillRecommender) {
		...
		this.#recommender =
			...
			new SkillRecommender({
				...
				judgmentDigest: judgmentConfigDigest(settings),
				// The live `judge` role chain: under an exact pin only the pinned
				// model; otherwise the credentialed judge candidates, then the
				// session model as a last resort. Nothing else is ever asked.
				judge: hooks => {
	...
	static createIf(session: ToolSession): RecommendSkillsTool | null {
		if (!cfgSkillsEnabled.get(session.settings)) return null;
		if (!cfgSkillsRecommendEnabled.get(session.settings)) return null;
