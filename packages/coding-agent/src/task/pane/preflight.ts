...
/** `task.paneBackend` value. */
export type PaneBackendSetting = SettingValueOf<typeof cfgTaskPaneBackend>;
...
export interface PaneCapabilityProbe {
	 ...
	 * extensions, MCP, and LSP the parent never granted.
	 */
...
export function readPaneBackendConfig(settings: Settings): PaneBackendConfig {
	const configured = cfgTaskHerdrSession.get(settings)?.trim();
	return {
		backend: cfgTaskPaneBackend.get(settings),
		session: configured || undefined,
		readyTimeoutMs: cfgTaskHerdrReadyTimeoutMs.get(settings),
		promptTimeoutMs: cfgTaskHerdrPromptTimeoutMs.get(settings),
		keepPane: cfgTaskHerdrKeepPane.get(settings),
	};
}
...
export async function preflightPaneBackend(options: PanePreflightOptions): Promise<PanePreflight> {
	...
	if (!/^\s+status:\s*running\s*$/m.test(status.stdout)) {
