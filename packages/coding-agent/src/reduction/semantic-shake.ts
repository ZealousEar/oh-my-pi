...
import { cfgCompactionSemanticShakeMaxRegionsPerCall } from "../session/context-settings";
...
export async function selectShakeRegions(input: ShakeSelectionInput): Promise<ShakeSelection> {
	...
	const maxRegionsPerCall = Math.max(1, cfgCompactionSemanticShakeMaxRegionsPerCall.get(input.settings));
