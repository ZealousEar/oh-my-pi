 ...
 * tokenizer, judge resolver, and {@link LoopMeter}.
 *
...
import { type DecisionProvenance, type LoopBudget, LoopMeter } from "../judgment/decision";
import { type ResolvedJudge, resolveJudge } from "../judgment/index";
...
/** Read the shared reduction policy from settings. */
export function resolveReductionPolicy(settings: Settings): ReductionPolicy {
	return {
		egress: settings.get("reduction.egress"),
		maxCallsPerPass: Math.max(0, settings.get("reduction.maxCallsPerPass")),
		maxLatencyMs: Math.max(0, settings.get("reduction.maxLatencyMs")),
		taskContextChars: Math.max(0, settings.get("reduction.taskContextChars")),
	};
}
 ...
 * admitted. The judge honours `providers.judgmentProvider`,
 * `providers.typesafeModel`, and `providers.judgmentFallback` exactly as the
 * task loops do; the meter bounds calls and wall clock by the shared policy.
 */
