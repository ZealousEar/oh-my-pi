...
import { cfgBashOutputPruning } from "../exec/settings";
import { judgeWithMeter, LoopBudgetExceeded } from "../judgment/decision";
import type { JudgeDeps } from "../judgment/index";
...
export interface PruneBashOutputInput {
	...
	sessionId?: string;
	sessionModel?: Model;
	/** Journals each judgment attempt on the session cost ledger (`journalJudgmentUsage`). */
...
export async function pruneBashOutput(input: PruneBashOutputInput): Promise<PruneBashOutputResult> {
	const pruning = cfgBashOutputPruning.get(input.settings);
	const mode = pruning.mode;
	...
	if (baselineTokens < Math.max(0, pruning.minTokens))
		return {
			visible: input.baseline,
	...
	if (mode === "semantic") {
		...
		if (admission && input.task) {
			const result = await pruneSemantically({
				...
				maxSegments: pruning.maxSegments,
				admission,
