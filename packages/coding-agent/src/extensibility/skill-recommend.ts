 ...
 *  - When the judge cannot be resolved (no registry, an exact `modelRoles.judge`
 *    pin that admits no candidate, no credentialed judge model), fails, or the
 *    loop budget is spent, the module degrades to a deterministic lexical
 *    overlap search and says so (`mode: "search"` plus a fallback reason). It
 *    never asks a substitute model the judge role chain did not admit.
...
export function readSkillRecommendConfig(settings: Settings): SkillRecommendConfig {
	return {
		enabled: cfgSkillsRecommendEnabled.get(settings),
		maxCandidatesPerRequest: clampWindowSize(cfgSkillsRecommendMaxCandidatesPerRequest.get(settings)),
		minRelevance: Math.max(0, Math.min(1, cfgSkillsRecommendMinRelevance.get(settings))),
		cacheEntries: Math.max(1, Math.trunc(cfgSkillsRecommendCacheEntries.get(settings))),
	};
}

/**
 * Digest of the configuration that decides which judge answers: the `judge`
 * role and its fallback chain (an exact pin is the pair `provider/id` + `[]`).
 * Part of the cache key so a re-routed judge never serves another's ranking.
 */
export function judgmentConfigDigest(settings: Settings): string {
	const chain = cfgRetryFallbackChains.get(settings).judge;
	return `${settings.getModelRole("judge") ?? ""}\u0000${Array.isArray(chain) ? chain.join("\u0001") : "-"}`;
}
...
export interface SkillRecommenderDeps {
	...
	/**
	 * Resolves the judge for one request. Throwing here (no registry) is a
	 * normal outcome and routes to the lexical fallback, as does a judge whose
	 * role chain admits no candidate when first asked.
	 */
	judge: (hooks: JudgeHooks) => DecisionJudge;
	/** Digest of the judgment configuration ({@link judgmentConfigDigest}); participates in the cache key. */
...
export class SkillRecommender {
	...
	async recommend(request: SkillRecommendRequest, options: RecommendOptions = {}): Promise<SkillRecommendResult> {
		...
		try {
			...
			// A chain that admitted no candidate fails before any transport is
			// asked, so every metered row is the opaque `api: "none"` placeholder:
			// the judge was unavailable, not wrong.
			const asked = meter.attempts.some(attempt => attempt.api !== "none");
			const reason =
				error instanceof LoopBudgetExceeded
					? `judgment budget spent (${error.limit})`
					: `${asked ? "judgment failed" : "judge unavailable"}: ${message}`;
