...
/**
 * Run the handler; when it fails with an `AUTOMATION_DENIED` naming this
 * prelude's surface, offer the user exactly the denied scope (60 minutes, or
 * once for this invocation) and retry a single time. Headless callers receive
 * the denial unchanged so their owner can preconfigure grants.
 */
	...
	try {
		...
		try {
			...
			const scope = grantAutomationScope(context.session, {
				...
				...(once ? { once: true, invocationId: context.toolCallId } : {}),
			});
			try {
				context.signal?.throwIfAborted();
