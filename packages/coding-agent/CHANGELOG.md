...
- Added `tab.task(goal, …)` to the `browser` Eval prelude: a goal-directed page loop that observes every visible control in one atomic pass, lets the judgment model pick one local candidate action per step, guards against stale targets and unknown mutations, refuses consequential controls (submit/pay/delete/…) unless `allowConsequential` is set, and independently verifies the goal before reporting `done`. Bounded by `browser.task.maxActions`, `browser.task.maxCalls`, and `browser.task.deadlineSec`.
- Added `app.new_tab: true` to `browser.open` for relay and `cdp_url` browsers: opens a fresh omp-owned tab in the background instead of adopting the user's visible tab, reports it as `(owned tab)`, lets `tab.task` run on it, and closes it on `browser.close`. Adopted tabs keep their never-closed behavior.
- Added `computer.task(goal, …)` to the `computer` Eval prelude: the same semantic loop over native accessibility trees of a named window, plus read-only `window.observe()` and Cua Driver detection in `computer.capabilities()` (`computer.driverBin` overrides the lookup). Bounded by `computer.task.maxActions`, `computer.task.maxCalls`, `computer.task.deadlineSec`, and `computer.task.allowConsequential`.
- Added the `recommend_skills` tool: ranks installed skills against a task with the judgment model (explicit `/skill:` or `skill://` mentions always included), falls back to a lexical ranking when the judge is unavailable, and reports provenance and cost. Configure with `skills.recommend.*`.
- Added the `semantic_find` tool (mounted under `xd://semantic_find`): ranks passages of named files or internal URLs by how well they answer a natural-language question, reports separately whether the answer is present, and re-reads every hit from disk before reporting it. Caps are configurable under `semanticFind.*`; there is no lexical fallback — use `grep` for exact matches.
- Added an optional HerdR pane backend for `task` subagents: `task.paneBackend` (`native` default, `auto` inside a HerdR pane, or `herdr` with `task.herdr.session`) runs a subagent in a visible pane with the same task card, `agent://` result artifact, and lifecycle states (`working`, `blocked-needs-user`, `idle`, `ready-for-review`, `needs-attention`). The child gets the native model chain, effort, tool allowlist, and recursion budget; only a result file that follows the delivery protocol (`resolved-model:` header, `<!-- omp-pane-result:end -->` sentinel) counts as success, and every pane result reports `resultSource` and `resolvedModelVerified`. Pane children cannot be messaged over `hub`, take no follow-up turns, and their usage stays in the pane session. `OMP_TASK_HERDR=0` disables it.
- Added `providers.typesafeModel` to pin the TypeSafe judgment model for reproducible decisions; every judgment reports which backend and model actually answered and whether its probabilities are native or synthetic.
- Added `providers.judgmentFallback` (`llm` default, `none`): with `none`, a failed TypeSafe judgment fails closed — the error reaches the feature and the failed attempt stays on its meter — instead of silently re-asking the online chat chain. Judges now also report every underlying transport attempt (the TypeSafe call and each chat candidate) so loops can account for failed and fallback attempts.
- Added opt-in `bash` output pruning (`bash.outputPruning.mode`: `off` default, `deterministic`, `semantic`), judged against the person's task: a bounded task context (original request, latest request, latest reply, standing requirement sentences) is read from the session; a stated retention or counting requirement keeps the whole output, no task context allows only content-free collapses (exact repeats with a `×N` count, blank runs), and otherwise recognised noise runs go by rule and — with `semantic` and `reduction.egress = selected` — a bounded judgment given the same task fields may omit further routine spans. Diagnostics (including `TypeError:`-style names and `[WARN]` tags), `path:line` references, counts, exit/status notices, artifact links, the head and tail, and structured output (JSON, diff, tables, listings, explicit file views) are always kept; the exact original is archived first and named once in a `[pruned output: M→K tokens; original: artifact://N]` footer; `details.reduction` records what was kept, omitted, and why. Non-zero exits use rules only; timeouts and recovery reads are never pruned.
- Added semantic shake: `/shake semantic` and the opt-in `semantic-shake` compaction method remove an old tool result behind the protect window only when a bounded judgment (`reduction.egress = selected`), given the person's task context, says it is no longer needed; error results, diagnostics, verification receipts, uncertain answers, and anything the judge could not be asked about (egress off, no judge, budget spent, oversized, no task context) stay verbatim, fenced blocks are left to the explicit `/shake elide`, every judged result gets a receipt (`ShakeResult.receipts`, `details.shake` on elided results), and when the context is still over the threshold the auto-compaction fallback names what was kept and why, what was elided, and the next method. Configure with `compaction.semanticShake.*`.
- Added the shared `reduction.*` settings (`egress` `off` default | `selected`, `maxCallsPerPass`, `maxLatencyMs`, `taskContextChars`) that gate what semantic reduction may read, send to the judgment backend, and spend per pass.
- Added a browser relay protocol identity: the relay advertises `OMP-Relay-Protocol` (200) / `relayProtocol` (503) = 2, refuses extensions older than 0.2.0 (`extensionIncompatible` in the 503 body, the bridge never becomes ready for them), and consumers treat relays without the field (older omp builds) as protocol 1. `OMP_BROWSER_RELAY_REQUIRE_PROTOCOL=<n>` makes the browser tool refuse a lower-protocol relay with the stop/kickstart fix; `OMP_BROWSER_RELAY_SUPERVISOR=launchd:<label>` starts the relay through that launchd job instead of spawning omp's own build (kickstart failure fails closed), and `OMP_BROWSER_RELAY_SUPERVISOR=none` is adopt-only (an already-serving relay is used, nothing is ever started). Present-but-malformed (including empty) values fail closed. `omp browser-relay serve --supervised` keeps retrying the bind every 2 s while any other relay owns the port, and `omp browser-relay install` reports a `chflags uchg`-locked canonical directory instead of a stack trace.
- Added relay ↔ Chrome-profile binding: the extension mints a durable per-install id (`chrome.storage.local`) and reports it in `hello`; the relay only becomes ready for the install named in `~/.omp/browser-relay/binding.json` (re-read on every hello and readiness query, so bind/unbind need no restart), keeps an unbound install connected but idle, closes any other install with websocket code 4403 (`profile-mismatch`; the extension shows `!` and backs off 60 s) without touching the bound connection, and reports the state in the 503 body (`profileBinding`, `reason`), the 200 body (`OMP-Profile-Binding`, `OMP-Profile-Fingerprint`), and `GET /omp/binding` (full install id only while unbound). The browser tool fails at once with the bind/rebind instructions when no bound install was ever connected, and retries for one dial window when the rejection followed a bound install that had been connected (a Chrome restart re-dialing). When a relay protocol is required, the adopted CDP connection itself must answer `Browser.getVersion.revision` with `omp-relay/2;binding=bound;fp=<fingerprint>` matching the pre-connect `/json/version` answer, or it is disconnected and reported — a relay swapped in between the check and the connection cannot be adopted.

### Changed

- Changed the judgment questions of `tab.task`, `computer.task`, `recommend_skills`, and `semantic_find` to follow the TypeSafe skill and docs: every `tab.task` target head now states its "assuming CLICK runs next" premise and offers a `none` option (the loop takes the next-ranked operation when a target head declines, noting the veto on the step), completion checks are yes/no nouls with `true`/`false` criteria over the fresh observation, instructions reference the state by path, `computer.task` no longer sends observation revision/digest bookkeeping to the judge, and `recommend_skills` asks one absolute `fits` noul per finalist in one extra request (reported as `applicable`; a finalist set whose best `fits` is under 0.30 is a no-match).
- Changed `omp browser-relay install` to always write the profile-independent `~/.omp/browser-relay/extension` (one Chrome loads one unpacked path; every omp profile now targets the same directory), and `getBrowserRelayDir()` accordingly.

### Fixed

- Fixed oversized tool results whose artifact save failed being truncated silently: the notice now says `Full output was not saved completely (artifact write failed)` instead of reading like a recoverable spill.
- Fixed `recommend_skills` listing a forced pick as `relevance 1.00` beside its own no-match verdict and letting a one-skill remainder window outrank every full window: a no-match result lists nothing, every window offers the judge a `none` option, windows are balanced so none holds a single skill, skill descriptions are normalised and capped at 300 characters before reaching the judge, echoed text is sanitised, and overlapping calls no longer chain their ledger entries off each other.
- Fixed judgment loops under-reporting cost when a chat judge rejected an answer and retried: every billed completion now stays on the loop's meter as a nested attempt, so `usage.attempts`, token totals, and cost include format retries.
- Fixed `semantic_find` discarding judgment provenance when the judge errors (the failed result now carries every attempt and its usage) and silently dropping later windows' finalists when `windows × limit` exceeded the request size; every window now seats `floor(passagesPerRequest / windows)` finalists and `coverage.finalistsDropped` reports the rest. Rendered snippets are sanitised, and absolute-path hits are re-read for verification like relative ones.
- Fixed the HerdR pane backend promoting a scraped pane transcript to a successful task result, accepting a stale or partial result file, ignoring aborts outside the prompt wait, launching the child with only the first model selector, no per-spawn effort, and an un-normalised tool list, and passing preflight with the omp integration installed for a different agent dir than the child uses. A sealed result file without the `resolved-model:` header now fails instead of being reported as a success with an unverified model, an explicit empty tool allowlist launches the child with `--no-tools` rather than its defaults, and restricted parent sessions (plan mode or an explicit tool allowlist) are refused at preflight because a fresh pane omp cannot honour the restriction.
- Fixed `tab.task` safety gaps found in review: consequential controls are now classified from every name they carry (accessible name, visible text, `aria-label`, `title`, the chosen `<select>` option) with an extended keyword list and mixed-script names failing closed; caller `values` match a field only by exact normalised label/placeholder/name (unmatched keys are reported as `unusedValues`, never typed); `done` requires a caller `expect` check to hold, not just the model's own completion judgment; an action with an `unknown` outcome is quarantined instead of re-dispatched; the deadline is checked before every page contact and remote call, per-call `maxActions`/`maxCalls` only tighten the configured defaults, text-helper completions are metered and listed with every other attempt in `result.attempts`; judge requests carry origin+path, `filled`/empty per control and action labels instead of URLs with query strings, field values, and typed text; SCROLL/WAIT are bound to the observed document; and helper-written values are rejected unless the goal or caller values ground them.
- Fixed shared MCP OAuth reuse accepting credentials for a different resource, authorization-server origin, or granted scope, while preserving DCR credentials whose embedded client correctly overrides stale per-profile refresh material.
