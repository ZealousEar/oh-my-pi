...
/**
 * Defends the bounded-run contract for background jobs: a job that goes silent
 * is surfaced to its owner exactly once, with enough context to inspect or kill
 * it through `proc://`, and never for jobs that are progressing, settled,
 * parked, hidden behind a foreground wait, or owned by someone without a route.
 */
describe("AsyncJobManager no-progress notice", () => {
	...
	test("warns exactly once for a silent running job, after the threshold", async () => {
		...
		// The notice names the 18.3 inspection and cancel actions, not a removed tool.
		expect(notices[0].text).toContain(`read proc://${jobId}`);
		expect(notices[0].text).toContain(`write proc://${jobId}/kill`);
		expect(notices[0].text).not.toContain("hub");
	...
	test("a foreground-backed job is watched only once promoted, counting silence from its last progress", async () => {
		const { manager, notices } = setup();
		const gate = Promise.withResolvers<string>();
		const jobId = manager.register("bash", "long foreground call", () => gate.promise, {
			ownerId: OWNER,
			foreground: true,
		});

		// Hidden behind the foreground wait: no background-job notice.
		vi.advanceTimersByTime(WARN_MS * 3);
		await settle();
		expect(notices).toHaveLength(0);

		// Promotion makes it a real background job; the accrued silence already
		// exceeds the threshold, so the warning fires on the next tick.
		expect(manager.backgroundJob(jobId)).toBe(true);
		vi.advanceTimersByTime(0);
		await settle();
		expect(notices).toHaveLength(1);
		expect(notices[0].text).toContain("no progress for 3s");
