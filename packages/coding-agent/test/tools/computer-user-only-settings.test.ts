...
describe("user-level-only computer settings", () => {
	...
	describe("through layered Settings", () => {
		...
		it("ignores a cloned repository's desktop settings while honouring the user's own", async () => {
			...
			expect(settings.get("computer.driverBin")).toBe("/opt/user/cua-driver");
			expect(settings.get("computer.task.allowConsequential")).toBe(false);
			expect(settings.get("computer.task.backend")).toBe("native");
			expect(settings.get("computer.cua.telemetry")).toBe("off");
			// Ordinary project keys still apply.
			expect(settings.get("computer.task.maxActions")).toBe(3);
		});
	});
});
