...
interface FakeJudge extends ResolvedJudge {
...
function fakeJudge(
	...
	kind: ResolvedJudge["kind"] = "online",
): FakeJudge {
	const calls: FakeCall[] = [];
	return {
		kind,
		label: kind === "typesafe" ? "typesafe/jev-1.12" : "tiny/smol/default",
		calls,
		async judge<Q extends Questions>(request: JudgmentRequest<Q>): Promise<JudgmentResult<Q>> {
			...
			return {
				api: kind === "typesafe" ? "typesafe" : "openai-codex-responses",
				provider: kind === "typesafe" ? "typesafe" : "openai-codex",
				model: kind === "typesafe" ? "jev-1.12" : "gpt-5.6-sol",
...
function recommender(judge: ResolvedJudge | (() => ResolvedJudge), config: Partial<SkillRecommendConfig> = {}) {
...
describe("SkillRecommender", () => {
	...
	it("falls back to lexical search with a reason when the judge fails", async () => {
		...
		).recommend({ task: "deploy the app to vercel", catalog });

		expect(result.mode).toBe("search");
		expect(result.fallbackReason).toContain("backend exploded");
	...
	it("carries provenance distinguishing a native distribution from a synthetic one", async () => {
		...
		const native = await recommender(fakeJudge(ids => ({ choice: ids[0], applicable: 0.9 }), "typesafe")).recommend({
			task: "alpha things",
			catalog,
		});
		...
		expect(native.attempts[0]).toMatchObject({ backend: "typesafe", distribution: "native" });
	...
	it("routes to lexical search when no judge can be resolved", async () => {
		...
		expect(result.mode).toBe("search");
		expect(result.fallbackReason).toContain("judge unavailable");
	});
});
