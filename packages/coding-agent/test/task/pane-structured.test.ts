...
describe("structured subagent on the pane backend", () => {
	it("launches the child with the native model chain, effort, tool allowlist, and spawn budget", async () => {
	...
	it("reports an idle child without a result file as a failure at the task boundary", async () => {
		vi.spyOn(discoveryModule, "discoverAgents").mockResolvedValue({ agents: [AGENT], projectAgentsDir: null });
