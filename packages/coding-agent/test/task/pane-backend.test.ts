...
describe("herdr pane backend", () => {
	test("creates a workspace, starts the agent, prompts it, and returns the artifact", async () => {
		fake = createFakeHerdr(happyScenario());
		const progress: AgentProgress[] = [];
		const outcome = await runPaneSubagent(paneOptions(fake, {}, progress));

		expect(fake.keys()).toEqual(["workspace create", "agent list", "agent start", "agent prompt", "workspace close"]);
	...
	test("a sealed result without the model header fails instead of being attributed to the requested selector", async () => {
	...
	test("forwards the whole model fallback chain, the thinking level, and the tool allowlist", async () => {
		fake = createFakeHerdr(happyScenario());
		await runPaneSubagent(
			paneOptions(fake, {
				modelPatterns: ["provider/primary", "provider/fallback"],
				thinkingLevel: ThinkingLevel.Max,
				toolNames: ["hub", "eval", "bash"],
			}),
		);
	...
	test("an explicit empty tool allowlist launches the child with --no-tools instead of its defaults", async () => {
		fake = createFakeHerdr(happyScenario());
	...
	test("hands the child its remaining recursion budget as a settings overlay", async () => {
		fake = createFakeHerdr(happyScenario());
	...
	test("inside a pane it splits a sibling and closes only that pane", async () => {
		fake = createFakeHerdr(happyScenario());
	...
	test("a tall caller pane splits down", async () => {
		fake = createFakeHerdr(
			happyScenario({
				"pane layout": {
					stdout: envelope({
	...
	test("injects pane environment and a unique agent name", async () => {
		fake = createFakeHerdr(
	...
	test("a blocked child keeps its pane and reports where to answer", async () => {
		fake = createFakeHerdr(
			happyScenario({
	...
	test("a settled blocked state is reported without closing the pane", async () => {
	...
	test("a child that never becomes ready fails with its pane retained", async () => {
		fake = createFakeHerdr(
			happyScenario({
	...
	test("a stalled prompt fails with its pane retained", async () => {
		fake = createFakeHerdr(
			happyScenario({
	...
	test("cancellation interrupts the child and releases the owned workspace", async () => {
		fake = createFakeHerdr(
			happyScenario({ "agent prompt": { sleepMs: 4_000, stdout: envelope({ state: "idle" }) } }),
		);
	...
	test("an abort while the result is settling is honoured, never scraped into a success", async () => {
	...
	test("an abort while the pane is being created starts no agent", async () => {
		fake = createFakeHerdr(happyScenario({ "workspace create": { sleepMs: 3_000, stdout: envelope({}) } }));
		const controller = new AbortController();
		const run = runPaneSubagent(paneOptions(fake, { signal: controller.signal }));
		setTimeout(() => controller.abort(), 300);
		const outcome = await run;

		expect(fake.keys()).toEqual(["workspace create"]);
		expect(outcome.result.aborted).toBe(true);
		expect(outcome.result.exitCode).toBe(1);
		expect(outcome.lifecycle).toBe("cancelling");
	});

	test("keepPane leaves the pane open after a successful run", async () => {
		fake = createFakeHerdr(happyScenario());
	...
	test("a child that goes idle without a result file fails; the transcript is evidence, not output", async () => {
		fake = createFakeHerdr(
			happyScenario({
				"agent prompt": { stdout: envelope({ state: "idle" }) },
				"agent read": { stdout: envelope({ text: "still investigating" }) },
	...
	test("a result file left over from an earlier run with the same id is never returned", async () => {
		fake = createFakeHerdr(
			happyScenario({
				"agent prompt": { stdout: envelope({ state: "idle" }) },
	...
	test("a result file without the closing sentinel is partial and fails", async () => {
		fake = createFakeHerdr(
			happyScenario({
				"agent prompt": {
					stdout: envelope({ state: "idle" }),
