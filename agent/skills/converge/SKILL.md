...
- NEVER poll. After a batch: one `hub jobs` identity snapshot ≤ 60 s, then `hub wait` / auto-delivered yields / independent work. Children push `BLOCKED:` / `NEEDS-APPROVAL:`; their yield is DONE.
- Blindness in L2/L3 round 1 is yours to keep: packets omit the sibling; the peer's name is never revealed in L2.
</critical>

## 0. Preflight

1. Read `references/orchestrator.md`; run its cell in your Python eval kernel (once per session).
2. `new_run(question, tier, experiments, constraints=…)` after §1–2 (needs the tier). It reads the six `converge-*` roles from the configured `modelRoles` and freezes model + effort into `manifest.expected` — config.yml is the only place a model or effort is chosen; a missing role (preset tombstone) fails here, before any spawn. Run id `<YYYYMMDD-HHMM>-<slug≤24>`; root `local://converge/<run-id>/`.
3. `jev_probe()`: one bool. Failure ⇒ `manifest.jev_backend = "orchestrator"`; you answer every card yourself (§7) and the report says so.
4. **Every spawn batch**: `result = task(**batch)` → `ids = register_spawn(r, phase, {requested name: job id from result})` = THIS batch's `{side|judge: id}`. Names are run/round/phase-specific and are never hub addresses; the allocated ids are. `spawn_ids(r, phase)` = the merged mapping across retries (peer release, `hub cancel`).
5. **Identity check** (every batch, debaters and judges): ≤ 60 s after `task`, `snap = await tool.hub(op="jobs", i="identity check")` → `exp = expected_for(r, ids)` for a debater batch, or `exp = expected_judges(stage, ids)` for a judge batch (one or the other, never merged; each raises on the other's roles) → `chk = identity_check(snap, exp, ids)` with `ids` = exactly what `register_spawn` returned for THIS batch: the expectation is built from those ids, and `identity_check` raises if the two do not name the same roles (a spawned id can never go unchecked). Label matching is not allowed (names repeat across rounds). Every row: identity = expected model, thinking = expected effort, not fallback. `chk["retry"]` (rows still `pending` — identity fields fill ~20–25 s after spawn) ⇒ do other work or `hub wait`, re-snapshot on the wake, check again; NEVER `time.sleep` in eval (30 s eval timeout). Mismatch ⇒ `hub cancel` the batch, `finish("incomplete_transport")`, tell the owner which role resolved to what.
...
{"questions": [{"id": "tier", "question": "Which converge tier for: <question>?", "recommended": 1, "options": [
  {"label": "L1 — direct exchange", "description": "Catches: <e.g. the obvious keep-alive vs re-spawn trade-off, fast>. Misses: framing anchoring on whichever side opens — for a one-way door on <X> that matters; hub loss degrades it to two monologues. 1 exchange, ~15–25 min. Astra medium / Fable high."},
...
ids = register_spawn(1, "converge", …) → for m in peer_messages(spawn_ids(1)): hub(**m)      ← releases peer contact (`PEER: <id>`)
  opener  --hub--> responder : position ≤500 words + cruxes
...
One round, no effort escalation, no reopen (objections are classified in place; a verdict-changing one becomes an open crux and drives the escalation ask). Peers wait for your `PEER:` message before any contact (ids are allocated by the harness; names repeat). Each side ≤ 3 hub messages, waits with `hub wait from:<id>`, gives up at 15 min ⇒ `blocked` ⇒ retry that side once: `spawn_round(1, sides=[X], attempt=2, extra=l1_transcript(1, <other side>))` → `ids = register_spawn(1, "converge", …)` → `identity_check(snap, expected_for(1, ids), ids)` → `peer_messages(spawn_ids(1))` (the surviving peer is revived by the send); a second failure voids the exchange (non-progress) and the escalation ask follows.

**L2 — blind drafts + ledger-mediated convergence** (default)

```text
round 1  spawn_round(1)  ← brief only, no peer name, "hub only to Main"; register_spawn; identity check
...
mapped = panel_verdicts(raw) [validate FULL 2.5.1, map BA back, complete/consistent per judge] → task(spawn_panel("conference", mapped["verdicts"])) → register_spawn → peer_messages(spawn_ids(0, "conference")) → ≤3 hub msgs each
...
| Identity row `pending` | `identity_check` → `retry: True` | Not a failure: identity fields fill ~20–25 s after spawn. `hub wait` (or independent work), re-snapshot on the wake, re-check. NEVER `time.sleep` in eval. |
| Identity mismatch / fallback | `identity_check` → `mismatches` | Cancel batch; `incomplete_transport`; name the role and what it resolved to. |
| Hub message loss | `failed` receipt; `hub wait` window elapses | L1 / conference: a peer never contacts anyone until your `PEER: <id>` message; it waits ≤ 15 min then yields `blocked` ⇒ retry once with `extra` = transcript so far. Child→Main pushes retry once after the next milestone. Yields auto-deliver; `agent://<id>` recovers a missed one. |
...
| Child `NEEDS-APPROVAL:` | hub push | Only you can `ask`: ask the owner, reply on hub to the child. Children never block a round on approval; experiments ride in the reply. |
...
5. Cleanup: `hub cancel` stragglers, `finish()`, `cleanup()` (worktrees, `manifest.status = done`).
